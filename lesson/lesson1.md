...
- Реализуйте методы `save, get, delete, clear, getAll, size` в классе `ru.agaev.webapp.storage.ArrayStorage`,
...
- Проверьте вашу реализацию с помощью классов `ru.agaev.webapp.MainArray.main()`
  и `ru.agaev.webapp.MainTestArrayStorage.main()`
...
1. В методе `clear()` обнуление массива предполагает обнуление (null) ячеек, где хранятся ru.agaev.webapp.model.Resume,
...
1. ru.agaev.webapp.model.Resume r — давайте переменным осмысленные имена, например resume. r допустимо в коротких циклах
   и лямбда-выражениях
